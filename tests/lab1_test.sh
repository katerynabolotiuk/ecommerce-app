...
# 1. Перевірка чи запущено контейнер з назвою 'ecommerce-app'
if [ $(docker ps -q -f name=ecommerce-app | wc -l) -eq 0 ]; then
  echo "❌ Error: Container named 'ecommerce-app' not found."
  exit 1
fi

# 2. Перевірка Health Check (200)
echo "Step 1: Checking /health (Expect 200)..."
STATUS=$(curl -s -o /dev/null -w "%{http_code}" localhost:3000/api/health)
...
sleep 3
STATUS_503=$(curl -s -o /dev/null -w "%{http_code}" localhost:3000/api/health)
